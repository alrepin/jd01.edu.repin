...
import ga.repin.education.course03.topic03.hw.school.service.StudentServiceV2;
...
@RestController
@RequestMapping(ga.repin.education.course03.topic03.hw.HwConstants.HW_ENDPOINT + "/student")
...
public class StudentControllerV2 {
    ...
    @PostMapping
    public ResponseEntity<StudentV2> createStudent(@RequestBody StudentV2 studentV2) {
        StudentV2 result = studentServiceV2.create(studentV2);
        if (result == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(studentV2);
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<StudentV2> updateStudent(@PathVariable long id, @RequestBody StudentV2 studentV2) {
        StudentV2 result = studentServiceV2.update(id, studentV2);
        if (result == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(result);
