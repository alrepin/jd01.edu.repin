...
import ga.repin.education.course03.topic03.hw.school.service.FacultyServiceV2;
...
@RestController
@RequestMapping(ga.repin.education.course03.topic03.hw.HwConstants.HW_ENDPOINT + "/faculty")
...
public class FacultyControllerV2 {
    ...
    @PostMapping()
    public ResponseEntity<FacultyV2> createFaculty(@RequestBody FacultyV2 facultyV2) {
        FacultyV2 result = facultyServiceV2.create(facultyV2);
        if (result == null) {
    ...
    public ResponseEntity<FacultyV2> readFacultyByID(@PathVariable long id) {
        FacultyV2 result = facultyServiceV2.get(id);
        if (result == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(result);
    ...
    public ResponseEntity<List<FacultyV2>> readFacultiesOptionalColorFilter(@RequestParam(value = "color", required = false) String color) {
        List<FacultyV2> result = new ArrayList<>(facultyServiceV2.getFaculties(color));
        if (result.isEmpty()) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<FacultyV2> updateFaculty(@PathVariable long id, @RequestBody FacultyV2 facultyV2) {
        FacultyV2 result = facultyServiceV2.update(id, facultyV2);
        if (result == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
        return ResponseEntity.status(HttpStatus.OK).body(result);
